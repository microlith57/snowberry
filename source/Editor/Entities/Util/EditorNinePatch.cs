...
public class EditorNinePatch {
    ...
    private readonly MTexture[,] nineSliceTexture;
    ...
    public void Draw(Vector2 pos, int width, int height, Color color) {
        ...
        // nr => 1 - max (if nr >= 0)
        static int moduloClamp(int nr, int max) => (nr % max) + 1;

        nineSliceTexture[0, 0].Draw(pos + new Vector2(0, 0), Vector2.Zero, color);
        nineSliceTexture[rows - 1, 0].Draw(pos + new Vector2(width - 8, 0), Vector2.Zero, color);
        nineSliceTexture[0, columns - 1].Draw(pos + new Vector2(0f, height - 8), Vector2.Zero, color);
        nineSliceTexture[rows - 1, columns - 1].Draw(pos + new Vector2(width - 8, height - 8), Vector2.Zero, color);

        for (int i = 1; i < tileWidth - 1; i++) {
            nineSliceTexture[moduloClamp(i - 1, rows - 2), 0].Draw(pos + new Vector2(i * 8, 0), Vector2.Zero, color);
            nineSliceTexture[moduloClamp(i - 1, rows - 2), columns - 1].Draw(pos + new Vector2(i * 8, height - 8f), Vector2.Zero, color);
        }

        for (int j = 1; j < tileHeight - 1; j++) {
            nineSliceTexture[0, moduloClamp(j - 1, columns - 2)].Draw(pos + new Vector2(0, j * 8), Vector2.Zero, color);
            nineSliceTexture[rows - 1, moduloClamp(j - 1, columns - 2)].Draw(pos + new Vector2(width - 8, j * 8), Vector2.Zero, color);
        }

        for (int k = 1; k < tileWidth - 1; k++)
            for (int l = 1; l < tileHeight - 1; l++)
                nineSliceTexture[moduloClamp(k - 1, rows - 2), moduloClamp(l - 1, rows - 2)].Draw(pos + new Vector2(k, l) * 8, Vector2.Zero, color);
    }
}